...
Object.assign(ColumnCompiler_Oracle.prototype, {
  ...
  increments({ primaryKey = true, constraintName = null } = {}) {
    ...
    if (constraintName) {
      return `integer not null constraint ${this.formatter.wrap(constraintName)} primary key`;
    }

    return 'integer not null primary key';
  },
  
  bigincrements({ primaryKey = true, constraintName = null } = {}) {
    this._createAutoIncrementTriggerAndSequence();

    if (primaryKey === false) {
      return 'number(20, 0) not null'; 
    }

    if (constraintName) {
      return `number(20, 0) not null not null constraint ${this.formatter.wrap(constraintName)} primary key`;
    }
