...
const trigger = {
  ...
  renameColumnTrigger: function(tableNameRaw, columnNameRaw, toRaw, formatter) {
    const triggerName = utils.generateCombinedName('autoinc_trg', tableNameRaw);
    const sequenceName = utils.generateCombinedName('seq', tableNameRaw);
  ...
  createAutoIncrementTrigger: function(tableNameRaw, formatter) {
    const triggerName = formatter.wrap(utils.generateCombinedName('autoinc_trg', tableNameRaw));
    const sequenceName = formatter.wrap(utils.generateCombinedName('seq', tableNameRaw));
    const tableName = formatter.wrap(tableNameRaw);
    return `DECLARE ` +
    `PK_NAME VARCHAR(200); ` +
    `BEGIN` +
  ...
  renameTableAndAutoIncrementTrigger: function(tableNameRaw, toRaw, formatter) {
    const triggerName = formatter.wrap(utils.generateCombinedName('autoinc_trg', tableNameRaw));
    const sequenceName = formatter.wrap(utils.generateCombinedName('seq', tableNameRaw));
    const toTriggerName = formatter.wrap(utils.generateCombinedName('autoinc_trg', toRaw));
    const toSequenceName = formatter.wrap(utils.generateCombinedName('seq', toRaw));
    const tableName = formatter.wrap(tableNameRaw);
    const to = formatter.wrap(toRaw);
    return `DECLARE ` +
    `PK_NAME VARCHAR(200); ` +
