...
const trigger = {
  renameColumnTrigger: function(
    ...
    const sequenceName = formatter.wrap(
      utils.generateCombinedName(logger, 'seq', tableNameRaw)
    );
  ...
  renameTableAndAutoIncrementTrigger: function(
    ...
    const toTriggerName = formatter.wrap(
      utils.generateCombinedName(logger, 'autoinc_trg', toRaw)
    );
    const toSequenceName = formatter.wrap(
      utils.generateCombinedName(logger, 'seq', toRaw)
    );
    const tableName = formatter.wrap(tableNameRaw);
    const to = formatter.wrap(toRaw);
